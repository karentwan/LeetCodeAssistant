...
function init() {
    ...
    if (url.includes("problemset/all")) {
        // 绑定事件
        var btn = $('*[role=navigation]');
        // 标题
        var header = $("*[role=table] .border-b *[role=row]");
        var rows = $("*[role=rowgroup]").children();
        // 开始遍历
        // console.log(objs);
        for(var i = 0; i < rows.length; i++) {
            ...
            var title = $(children[1]).text();
            var strs = title.split(".");
            var level = $(children[4]).text();
            var id = strs[0];
            var score = "Not Score"
            if (objs != undefined && objs.hasOwnProperty(id)) {
                score = objs[id]["rating"];
                $(children[4]).text(score);
            }
        }
        var observer = new MutationObserver(homePageScore);
        var config = {
            attributes: true,
            characterData: true,
            childList: true,
